...
//	/*
//	     模式：
//	         r：read只读模式，不能写如果强行写出，数据是无法写到本地文件的
//	         w：write只写模式
//			 细节1：如果文件不存在，那么会把文件创建出来，但是要保证前面的文件夹都是存在的
//			 细节2：如果文件已经存在，会把文件清空
//			 细节3：上面的创建或者清空，都是fopen这个函数底层干的事情
//	         a：append追加写出模式
//			 细节1：如果文件不存在，那么会把文件创建出来，但是要保证前面的文件夹都是存在的
//			 细节2：如果文件已经存在，不会把文件清空，而是接着写（续写模式）
//			 细节3：上面的创建或者清空，都是fopen这个函数底层干的事情
//	*/    
//
//
//		//1.打开文件
//	FILE* file = fopen("C:\\Users\\LENOVO\\Desktop\\a.txt.txt", "a");
//
//
//	//2.写出数据
